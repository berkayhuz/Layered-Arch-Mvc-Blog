    ...
    public class UserController : Controller
	{
		...
		public async Task<IActionResult> Add(UserAddDto userAddDto)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					toast.AddSuccessToastMessage(Messages.AppUser.Add(userAddDto.Email), new ToastrOptions { Title = "İşlem Başarılı" });
			...
			if (user != null)
			{
				...
				if (ModelState.IsValid)
				{
					...
					if (validation.IsValid)
					{
						...
						if (result.Succeeded)
						{
							toast.AddSuccessToastMessage(Messages.AppUser.Update(userUpdateDto.Email), new ToastrOptions { Title = "İşlem Başarılı" });
							return RedirectToAction("Index", "User", new { Area = "Admin" });
						}
						else
						{
		...
		public async Task<IActionResult> Delete(Guid userId)
		{
			...
			{
				toast.AddSuccessToastMessage(Messages.AppUser.Delete(result.email), new ToastrOptions { Title = "İşlem Başarılı" });
				return RedirectToAction("Index", "User", new { Area = "Admin" });
			}
			return NotFound();
		}
